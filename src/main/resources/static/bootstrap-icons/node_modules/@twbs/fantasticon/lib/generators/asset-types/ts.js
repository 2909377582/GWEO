...
const generator = {
    generate: async ({ name, codepoints, assets, formatOptions: { ts } = {} }) => {
        ...
        const generateKind = (Boolean(ts?.types?.length)
            ...
            .map(kind => ({ [kind]: true }))
            .reduce((prev, curr) => Object.assign(prev, curr), {});
        const enumName = ts?.enumName || (0, change_case_1.pascalCase)(name);
        const constantName = ts?.constantName || `${(0, change_case_1.constantCase)(name)}_CODEPOINTS`;
        const literalIdName = ts?.literalIdName || `${(0, change_case_1.pascalCase)(name)}Id`;
        const literalKeyName = ts?.literalKeyName || `${(0, change_case_1.pascalCase)(name)}Key`;
